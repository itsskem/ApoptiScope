...
def main():
        ...
        # 1️⃣ User input
        data_folder = input("📂 What is the name of the dataset folder containing the spheroid assay images? ").strip()
        if not os.path.isdir(data_folder):
            print(f"❌ Folder '{data_folder}' does not exist.")
            return
        user_filename = input("💾 Name of CSV file to save results (e.g. results.csv): ").strip()
    
        print("\n✅ Starting ApoptiScope analysis!\n")
    
        # 2️⃣ Get all .tif/.tiff files
        all_files = get_all_files(data_folder)
        print(f"✅ Found {len(all_files)} image files in folder.\n")
    
        # 3️⃣ Find slices with apoptosis signal
        apoptosis_slice_ids = find_apoptosis(all_files)
        print(f"✅ Identified {len(apoptosis_slice_ids)} slices with apoptosis signal.\n")
        if not apoptosis_slice_ids:
            print("⚠️ No slices detected with apoptosis signal. Exiting.")
            return
    
        # 4️⃣ Collect matching channels
        DAPI_channels, apoptosis_channels, multi_channels = get_matching_images(all_files, apoptosis_slice_ids)
        print(f"✅ Channels collected: DAPI={len(DAPI_channels)}, Apoptosis={len(apoptosis_channels)}, Multi={len(multi_channels)}\n")
    
        if not DAPI_channels or not apoptosis_channels:
            print("⚠️ Missing necessary channels for segmentation. Exiting.")
            return
        ...
        if not segmented_masks or not dapi_masks:
            print("⚠️ Segmentation failed for one or more channels. Exiting.")
            return
    
        # 6️⃣ Quantify apoptosis
        results = quantify_apoptosis(segmented_masks, dapi_masks, apoptosis_channels, multi_channels)
        if not results:
            print("⚠️ No quantification results generated. Exiting.")
            return
    
        # 7️⃣ Save raw results
        save_results(results, user_filename)
    
        # 8️⃣ Filter results (apoptosis_area >= 20000)
        df = pd.read_csv(user_filename)
        filtered_df = df[df["apoptosis_area"] >= 20000]
        if filtered_df.empty:
            print("⚠️ No rows passed apoptosis_area >= 20000 filter. Exiting.")
            return
    
        # 9️⃣ Analyze results vs controls
        control_ids = ['s01', 's02', 's03', 's04', 's05']
        analyzed_df = analyzing_results(filtered_df, user_filename, control_ids)
    
        # 10️⃣ Show treated images
